...
void logic (int w, int h, char** input){
  ...
  if(input[0][0] != '9'){
		fprintf(fp, "(assert (and");
		fprintf(fp, " (= p%d-%d 1)", 0, 0);
		fprintf(fp, " (= p%d-%d 1)", 1, 0);
		fprintf(fp, " (= p%d-%d 1)", 0, 1);
		fprintf(fp, " (= p%d-%d 1)", 1, 1);
		fprintf(fp, "))\n");
	}
  else if(input[0][0] != '0'){
		fprintf(fp, "(assert (and");
		fprintf(fp, " (= p%d-%d 0)", 0, 0);
		fprintf(fp, " (= p%d-%d 0)", 1, 0);
		fprintf(fp, " (= p%d-%d 0)", 0, 1);
		fprintf(fp, " (= p%d-%d 0)", 1, 1);
		fprintf(fp, "))\n");
	}
	else if(input[0][0] != '?'){
  ...
  if(input[0][w-1] != '9'){
		fprintf(fp, "(assert (and");
		fprintf(fp, " (= p%d-%d 1)", w-1, 0);
		fprintf(fp, " (= p%d-%d 1)", w-1, 1);
		fprintf(fp, " (= p%d-%d 1)", w-2, 0);
		fprintf(fp, " (= p%d-%d 1)", w-2, 1);
		fprintf(fp, "))\n");
	} 
  else if(input[0][w-1] != '0'){
		fprintf(fp, "(assert (and");
		fprintf(fp, " (= p%d-%d 0)", w-1, 0);
		fprintf(fp, " (= p%d-%d 0)", w-1, 1);
		fprintf(fp, " (= p%d-%d 0)", w-2, 0);
		fprintf(fp, " (= p%d-%d 0)", w-2, 1);
		fprintf(fp, "))\n");
	}
	else if(input[0][w-1] != '?'){
  ...
  if(input[h-1][0] != '9'){
    fprintf(fp, "(assert (and");
    fprintf(fp, " (= p%d-%d 1)", 0, h-1);
    fprintf(fp, " (= p%d-%d 1)", 1, h-1);
    fprintf(fp, " (= p%d-%d 1)", 0, h-2);
    fprintf(fp, " (= p%d-%d 1)", 1, h-2);
		fprintf(fp, "))\n");
  }
  else if(input[h-1][0] != '0'){
    fprintf(fp, "(assert (and");
    fprintf(fp, " (= p%d-%d 0)", 0, h-1);
    fprintf(fp, " (= p%d-%d 0)", 1, h-1);
    fprintf(fp, " (= p%d-%d 0)", 0, h-2);
    fprintf(fp, " (= p%d-%d 0)", 1, h-2);
		fprintf(fp, "))\n");
  }
  else if(input[h-1][0] != '?'){
  ...
  if(input[h-1][w-1] != '9'){
    fprintf(fp, "(assert (and");
	  fprintf(fp, " (= p%d-%d 1)", h-1, w-1);
	  fprintf(fp, " (= p%d-%d 1)", h-1, w-2);
	  fprintf(fp, " (= p%d-%d 1)", h-2, w-1);
	  fprintf(fp, " (= p%d-%d 1)", h-2, w-2);
	  fprintf(fp, "))\n");
	}
  else if(input[h-1][w-1] != '0'){
    fprintf(fp, "(assert (and");
	  fprintf(fp, " (= p%d-%d 0)", h-1, w-1);
	  fprintf(fp, " (= p%d-%d 0)", h-1, w-2);
	  fprintf(fp, " (= p%d-%d 0)", h-2, w-1);
	  fprintf(fp, " (= p%d-%d 0)", h-2, w-2);
	  fprintf(fp, "))\n");
	}
	else if(input[h-1][w-1] != '?'){
